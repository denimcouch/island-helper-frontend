...
function BugModal({ bug }) {
  ...
  return (
    ...
    <p>Time Available: {available.time} </p>
    <p>Months Available:</p>
      <ul>
    <li>Northern Hemisphere: {mapMonths(available['month-array-northern'])}</li>
    <li>Southern Hemisphere: {mapMonths(available['month-array-southern'])}</li>
      </ul>
    <p>Price: {bug.price} bells / {bug.price_flick} bells (Flick) </p>
